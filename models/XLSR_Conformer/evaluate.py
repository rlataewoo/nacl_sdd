...
def main():
    ...
    parser.add_argument('--track', type=str, default='LA', help='ASVspoof 2019 LA or DF')
    ...
    if variable:
        score_path = os.path.join(args.training.log_dir, 'Scores_VL', track)
    else:
        score_path = os.path.join(args.training.log_dir, 'Scores', track)
    
    if not os.path.exists(score_path):
        os.makedirs(score_path, exist_ok=True)

    file_eval = read_metadata(dir_meta = args.data.path_eval_protocol, is_eval=True)
    print('no. of eval trials',len(file_eval))

    eval_set = ASVspoofDataset_eval(
        list_IDs = file_eval, 
        base_dir = args.data.path_eval,
        variable = variable
    )
