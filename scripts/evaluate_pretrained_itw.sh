...
if [ "${USE_VARIABLE}" = "True" ]; then
    ...
    poetry run \
        ...
        models/${MODEL}/evaluate_itw.py --config pretrained/LA/${VERSION}/config.yaml --variable True
else
    echo "Using fixed length"
    poetry run \
        accelerate launch --config_file ${ACCEL_CONFIG} \
        models/${MODEL}/evaluate_itw.py --config pretrained/LA/${VERSION}/config.yaml
fi

if [ "${USE_VARIABLE}" = "True" ]; then
    poetry run python common/eval/evaluate_in_the_wild.py \
        pretrained/LA/${VERSION}/Scores_VL_ITW/scores.txt \
        datasets/keys/ITW/meta.csv
else
    poetry run python common/eval/evaluate_in_the_wild.py \
        pretrained/LA/${VERSION}/Scores_ITW/scores.txt \
        datasets/keys/ITW/meta.csv
fi
