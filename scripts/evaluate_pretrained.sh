...
if [ "${USE_VARIABLE}" = "True" ]; then
    ...
    poetry run \
        ...
        models/${MODEL}/evaluate.py --config pretrained/${TRACK}/${VERSION}/config.yaml --track ${TRACK} --variable True
else
    echo "Using fixed length"
    poetry run \
        accelerate launch --config_file ${ACCEL_CONFIG} \
        models/${MODEL}/evaluate.py --config pretrained/${TRACK}/${VERSION}/config.yaml --track ${TRACK}
fi

if [ "${USE_VARIABLE}" = "True" ]; then
    poetry run python common/eval/main.py \
        --cm-score-file pretrained/${TRACK}/${VERSION}/Scores_VL/${TRACK}/scores.txt \
        --track ${TRACK} --subset eval
else
    poetry run python common/eval/main.py \
        --cm-score-file pretrained/${TRACK}/${VERSION}/Scores/${TRACK}/scores.txt \
        --track ${TRACK} --subset eval
fi
